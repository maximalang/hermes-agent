...
def _with_terminal_receipt(
    metadata: Optional[dict], *, outcome: str, summary: Optional[str] = None,
    error: Optional[str] = None,
) -> dict:
    """Attach the stable, machine-readable handoff generated by the DB layer.

    The worker supplies evidence in normal handoff metadata when available, but
    never needs to know this internal envelope. Recovery outcomes always retain
    a durable diagnostic reference (run/error) rather than pretending success.
    """
    value = dict(metadata) if isinstance(metadata, dict) else {}
    existing = value.get("receipt")
...
def _end_run(
    ...
    if run_id is None:
        return None
    metadata = _with_terminal_receipt(metadata, outcome=outcome, summary=summary, error=error)
    conn.execute(
        """
        UPDATE task_runs
           SET status        = ?,
