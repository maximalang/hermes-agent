...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _send_text_locked(
        ...
        try:
            ...
            return await self._send_chunks(
                chat_id, chunks, delivered, reply_to, metadata, error_types, html_mode=html_payload)
        except Exception as e:
    ...
    async def _send_chunks(
        ...
        metadata: Optional[Dict[str, Any]], error_types: tuple, html_mode: bool = False) -> SendResult:
        ...
        ``html_mode`` routes each chunk through parse_mode=HTML instead of MarkdownV2."""
        thread_id = self._metadata_thread_id(metadata)
        ...
        for chunk in chunks:
            outcome = await self._send_chunk_with_retries(
                chat_id, chunk, len(delivered), reply_to, metadata, thread_id, used_thread_fallback,
                error_types, html_mode=html_mode)
